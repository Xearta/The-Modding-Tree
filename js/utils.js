...
function getStartPlayer() {
  ...
  for (layer in layers) {
    playerdata[layer] = {};
    if (layers[layer].startData) playerdata[layer] = layers[layer].startData();
    else playerdata[layer].unlocked = true;
    playerdata[layer].buyables = getStartBuyables(layer);
    if (playerdata[layer].clickables == undefined)
      playerdata[layer].clickables = getStartClickables(layer);
...
function getStartClickables(layer) {
  ...
  if (layers[layer].clickables) {
    for (id in layers[layer].clickables) if (!isNaN(id)) data[id] = '';
  }
  return data;
}
...
function getStartChallenges(layer) {
  ...
  if (layers[layer].challenges) {
    for (id in layers[layer].challenges) if (!isNaN(id)) data[id] = 0;
  }
  return data;
}
...
function fixData(defaultData, newData) {
  for (item in defaultData) {
    if (defaultData[item] == null) {
      ...
      !!defaultData[item] &&
      defaultData[item].constructor === Object
    ) {
      if (
      ...
      if (
        ...
        confirm(
          "Invalid value found in player, named '" +
            item +
            "'. Please let the creator of this mod know! Would you like to try to auto-fix the save and keep going?"
        )
      ) {
        NaNalert = true;
...
function changeTheme() {
  ...
  document.body.style.setProperty(
    '--background_tooltip',
    aqua ? 'rgba(0, 15, 31, 0.75)' : 'rgba(0, 0, 0, 0.75)'
  );
...
function layerunlocked(layer) {
  ...
  return (
    ...
    (player[layer].unlocked ||
      (tmp[layer].baseAmount.gte(tmp[layer].requires) && tmp[layer].layerShown))
  );
}
...
function updateMilestones(layer) {
  for (id in layers[layer].milestones) {
    if (
      !player[layer].milestones.includes(id) &&
      layers[layer].milestones[id].done()
    )
...
function updateAchievements(layer) {
  for (id in layers[layer].achievements) {
    if (
      !isNaN(id) &&
      !player[layer].achievements.includes(id) &&
  ...
  if (tmp[layer].type == 'normal')
    ...
    return `${
      ...
      tmp[layer].roundUpCost
		...
		`;
  else if (tmp[layer].type == 'none') return '';
  else return layers[layer].prestigeButtonText();
}
